...
insert_entered_row_to_linked_list(node **head, char *row){
  ...
  insertFirst (head, atoi(row_values[0]), atoi(row_values[1]), row_values[2]);
...
get_id_and_find_element_by_id (node **head) {
  ...
  if (check_if_str_consist_of_digits (entered_row)){
    node *foundLink = find_by_id (*head,atoi(entered_row));
    if (NULL == foundLink) {
...
execute_main_menu_entered_cmd(char *entered_str, node **head){
  ...
  if ((strcmp(entered_str,"exit") == 0))
    return 1; /* TODO: rewrite return 1*/
  ...
  case '3': {
    node *deleted_node = deleteFirst(head);
  ...
  case '5':
    sort (*head, SORT_BY_ID);
    printf ("Sorting by id was done.\n");
    break;

  case '6':
    sort (*head, SORT_BY_YEARS);
    printf ("Sorting by years was done.\n");
    break;

  case '7':
    sort (*head, SORT_BY_NAME);
